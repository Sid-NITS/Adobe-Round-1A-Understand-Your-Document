...
SCORE_THRESHOLD = 4
START_PAGE = 1  # skip first page (title) unless not present!
...
def extract_pdf_title(pdf_path, weights=WEIGHTS_TITLE, box_factor=BOX_EXPANSION, dpi=DPI):
    ...
    for s in spans_sorted:
        ...
        txt = s['text']
        x0, y0, x1, y1 = s['bbox']
        if txt and txt[0].isdigit() and '.' in txt.split()[0]:
        ...
        if abs((x0 + x1) / 2 - page_w / 2) < page_w * 0.15:
            score += weights['centered']
        if (y0 - y_prev) > s['size'] * 1.5:
            score += weights['spacing']
        if abs(s['size'] - body_med) < 0.5:
            score += weights['body_penalty']
    ...
    x0c = max(0, int(cx - nw / 2))
    y0c = max(0, int(cy - nh / 2))
    x1c = min(img.shape[1], int(cx + nw / 2))
    y1c = min(img.shape[0], int(cy + nh / 2))
...
def extract_blocks(doc):
    spans = []
    sid = 0
    start_pg = START_PAGE if len(doc) > START_PAGE else 0
    for pi in range(start_pg, len(doc)):
...
def score_span(span, baseline, page_w, med_sp):
    ...
    if any(p.match(t) for p in NUMBERING_PATTERNS):
        score += WEIGHTS_HEAD['numbering']
    if span['font_size'] >= maxb + 1.5:
        score += WEIGHTS_HEAD['font_size']
    if span['bold']:
        score += WEIGHTS_HEAD['bold']
    if t.isupper() and len(t) > 5:
        score += WEIGHTS_HEAD['all_caps']
    w = span['x1'] - span['x0']
    if abs((page_w - w) / 2 - span['x0']) < 10:
        score += WEIGHTS_HEAD['centered']
    sp_val = span.get('spacing_above', 0) or 0
    if sp_val > med_sp.get(span['page'], 0) * 1.5:
        score += WEIGHTS_HEAD['spacing']
    if round(span['font_size']) in baseline:
        score += WEIGHTS_HEAD['body_penalty']
    return score
...
def assign_heading_levels(cands):
    ...
    return [{'level': lvl[c['font_size']], 'text': c['text'], 'page': c['page'] - 1} for c in cands]

def extract_headings_from_pdf(pdf_path):
    doc = fitz.open(pdf_path)
    title = extract_pdf_title(pdf_path).strip().lower()
    toc = extract_toc(doc)
    if toc:
        filtered_toc = [h for h in toc if h['text'].strip().lower() != title]
        return filtered_toc
    spans = extract_blocks(doc)
    if not spans:
        return []
    start_pg = START_PAGE if len(doc) > START_PAGE else 0
    page_w = doc.load_page(start_pg).rect.width
    med_sp = {}
    ...
    for s in spans:
        s['score'] = score_span(s, baseline, page_w, med_sp)
    cands = [s for s in spans if s['score'] >= SCORE_THRESHOLD]
    groups = defaultdict(list)
    for s in spans:
        groups[s['page']].append(s)
    merged = merge_colinear_text(cands, groups)
    merged = merge_multiline(sorted(merged, key=lambda s: (s['page'], s['y0'])))
    headings = assign_heading_levels(merged)
    filtered_headings = [h for h in headings if h['text'].strip().lower() != title]
    ...
    for file_name in os.listdir(INPUT_DIR):
        if file_name.endswith('.pdf'):
            ...
            try:
                ...
                out_name = f"result_{os.path.splitext(file_name)[0]}.json"
                out_path = os.path.join(OUTPUT_DIR, out_name)
                with open(out_path, 'w', encoding='utf-8') as f:
                    json.dump(result, f, ensure_ascii=False, indent=2)
                print(f"✅ Saved extraction result to '{out_name}' ({len(outline)} headings)")
